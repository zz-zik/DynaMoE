...
from models.decoder import DynamicMoE
from models.encoder.vmamba import CVSSDecoderBlock
...
class MambaDecoder(nn.Module):
    def __init__(self,
                 ...
                 classes=['building', 'vegetation', 'road'],
                 dropout_ratio=0.1,
                 embed_dim=96,
                 align_corners=False,
                 patch_size=4,
                 ...
                 use_moe=False,  # 是否使用 MOE 模型
                 expert_hidden_dim=128,  # 专家隐藏层维度
                 gate_hidden_dim=256,  # 门控隐藏层维度
                 new_classes=None,  # 新类别
                 **kwargs):
        super().__init__()

        self.classes = classes
        self.num_classes = len(classes)
        self.use_moe = use_moe  # 是否使用 MOE 模型
        self.new_classes = new_classes  # 新类别
        ...
        # 使用参数控制使用 MOE 模型
        if self.use_moe:
            self.moe_head = DynamicMoE(
                input_channels=embed_dim,
                initial_expert_types=self.classes,
                expert_hidden_dim=expert_hidden_dim,
                gate_hidden_dim=gate_hidden_dim
            )
            if self.new_classes:
                self.moe_head.freeze_existing_experts()
    ...
    def add_class(self, new_class: str):
        """
        动态添加一个新的类别及其对应的专家到 MOE 模型中。

        Args:
            new_class (str): 要添加的新类别的名称，如 'building'、'forest' 等。
        """
        if not self.use_moe:
            raise RuntimeError("当前未使用 MOE 模型，无法添加新类别。")

        # 调用 DynamicMoE 的 add_expert 方法
        self.moe_head.add_expert(new_class)

        # 更新 self.classes 和 self.num_classes
        if new_class not in self.classes:
            self.classes.append(new_class)
            self.num_classes = len(self.classes) + 1

        # print(f"已成功添加类别：{new_class}，当前类别数为：{self.num_classes - 1}")
    ...
    def up_x4(self, x, pz):
        ...
        if hasattr(self, 'use_moe') and self.use_moe:
            # 输入为 [B, C, H, W]
            x = self.moe_head(x, return_gates=True)  # 返回字典 {'prediction': ...}
        else:
            x = self.output(x)
            x = {'prediction': x}
